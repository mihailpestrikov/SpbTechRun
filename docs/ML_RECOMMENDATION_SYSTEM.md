...
## Интеграция скидок в рекомендации

### Источник данных

Датасет `promos.csv` содержит информацию о скидках:
- `promo_id` — ID акции
- `offer_id` — ID товара
- `promo_type` — тип акции ("bonus card" и др.)
- `discount_price` — цена со скидкой
- `start_date`, `end_date` — период действия

### Как скидки влияют на рекомендации

**1. Буст товаров со скидкой**
- Товары с активной скидкой получают множитель 1.2-1.5 при ранжировании
- Чем больше процент скидки, тем выше буст

**2. Отдельный блок "Выгодные предложения"**
- На главной и в карточке товара показывать блок со скидками
- Персонализация: скидки на товары из интересов пользователя

**3. Срочность (urgency)**
- "Скидка заканчивается через 3 дня" — дополнительный буст
- Фильтр товаров с истекающими скидками

### Реализация

**Backend:**
- Таблица `promos` с FK на `products`
- API: `GET /api/promos/products` — товары со скидками
- При запросе товара возвращать `discount_price` если есть активная акция
- Кэш активных скидок в Redis (обновлять раз в час)

**ML-сервис:**
- При генерации рекомендаций проверять наличие скидки
- Добавлять `has_discount`, `discount_percent` в признаки для ранжирования
- Отдельный эндпоинт для "выгодных рекомендаций"

```python
def apply_promo_boost(candidates, active_promos):
    for candidate in candidates:
        if candidate.id in active_promos:
            promo = active_promos[candidate.id]
            discount_percent = (candidate.price - promo.discount_price) / candidate.price
            # Буст пропорционален скидке: 10% скидка = 1.1x, 30% = 1.3x
            candidate.score *= (1 + discount_percent)
    return candidates
```
